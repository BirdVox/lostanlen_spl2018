...
%        Steve~Kelling,
%        Richard~F.~Lyon,
%		and Juan~Pablo~Bello% <-this % stops a space
...
R. A. Saurous and R. F. Lyon are with Google LLC.}% <-this % stops a space
}
...
% The paper headers
\markboth{IEEE Signal Processing Letters,~Vol.~X, No.~Y, Submitted~April~2018}%
...
\IEEEPARstart{T}{he} human auditory system exhibits a remarkable ability to identify cues from distant sources, despite the presence of environmental absorption, reverberation, and noise \cite{meyer2013plos}.
...
Indeed, most available datasets for training these systems were recorded under well-controlled experimental conditions: a high signal-to-noise ratios (SNR above \SI{30}{\deci\bel}) and a microphone placed near the source, \ie{} \SI{1}{m} or less. % CITE WSJ, CHIME, TIMIT
Consequently, a
% is exposed to 
...
% differences in impedance and dynamic range
% ASR: clean data. Cite WSJ, CHIME, TIMIT datasets
% AED: small number of independent recording conditions.


% at a later stage of the acoustic frontend.
% Cite Delcroix.

% Two solutions to mitigate bias.
% 1) active noise reduction at the acquisition stage by multiphonic recordings. Incurs a cost in hardware.
% 2) at the learning stage by context-adaptative deep neural networks (CA-DNN), data augmentation by degradation (cite muda and ADT), and domain-adversarial training. Incurs a cost in software.

% Whereas the automatic transcription of speech from ``clean'' audio streams, such as news broadcasts or , is about to become a solved problem, other application settings in ASR, such as the
% automatic subtitling of classroom lectures, or aerospatial communications, remains out of reach of currently existing ASR systems.

% The simplicity of PCEN: virtually no computational cost with respect to a logmelspec baseline.
...
%where $T$ is proportional to the time scale of local stationarity in background noise. 
%The features feeding the filter-generating network should carry information about background noise while being invariant about the class of interest $\boldsymbol{y}$ in the foreground.
%In the context of avian flight call detection, we design these so-called auxiliary features as seven long-term order statistics (median, quartiles, deciles, and centiles) summarizing the spectral envelope in the acoustic front end -- either log-mel or PCEN -- over windows of duration $T$, where $T$ is proportional to the time scale of local stationarity in background noise.
%After cross-validating the parameter $T$ as a geometric progression ranging between one second and two hours, we found that all values above $5$ minutes led to a background estimator of sufficiently low variance to avoid overfitting.
% Remark: these features could be the output of an RNN.
